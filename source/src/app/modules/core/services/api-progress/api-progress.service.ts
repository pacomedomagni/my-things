...
import { catchError, map, mergeMap, retryWhen, take, tap, timeout, withLatestFrom } from "rxjs/operators";
...
export class ApiProgressService {
	...
	constructor(
		...
		private notificationService: NotificationService) {
			ApiProgressService.sessionGuid = ApiProgressService.generateGuid();
		 }
	...
	private request<T extends any>(
		...
		const traceId = ApiProgressService.sessionGuid;
		...
		const result$ = request$.pipe(
			...
			mergeMap(([request, timeoutValue]) => {
				...
				return this.httpClient.request(request.method, request.url, requestOptions).pipe(tap(event => {
					...
					retryWhen(ApiProgressService.genericRetryStrategy({ maxRetryAttempts: options.retries, retryDelay: options.retryDelay })),
